...
class CompileTimeSurfaceFactory[Q <: Quotes](using quotes: Q):
  import quotes.*
  import quotes.reflect.*
  ...
  private def hasSecretAnnotation(s: Symbol): Boolean = false
  private def hasRequiredAnnotation(s: Symbol): Boolean = false
